...
if(!empty( $_POST ) && isset($_POST['btnSubmitStyleOptions']) && check_admin_referer($this->updateAppearanceNonce)){
    ...
    if (isset($_POST['descriptionSegments'])) {
        update_option($this->pluginOptions['descriptionSegments'], $_POST['descriptionSegments']);
    }
     ...
     <div class="section section-0">
         ...
         <div class="data">
            ...
            <select name="descriptionSegments">
                <?php for( $i = 1; $i <= 5; $i++){ ?>
                    <option value="<?php echo $i;?>" <?php echo ($i == $descriptionSegments) ? "selected='selected'" : ""; ?>><?php echo $i ?></option> 
                <?php } ?>
            </select>
        </div>
